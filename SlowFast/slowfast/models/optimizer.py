...
def get_optimizer(model, layer_decay=1.0, max_depth=12, zero='none'):
    def _get_layer_id(n):
        if layer_decay == 1.0:
            return 0
        if n == 'cls_token' or n.startswith('pos_embed') or n.startswith('patch_embed'):
            lid = 0
    ...
    not_regularized = defaultdict(list)
    patch_embed_regularized = defaultdict(list)
    patch_embed_not_regularized = defaultdict(list)
    ml = len('module.')
    bl = len('backbone.')
    for name, p in model.named_parameters():
        if p.requires_grad:
            name = name[ml:] if name.startswith('module.') else name
            name = name[bl:] if name.startswith('backbone.') else name
            layer_id = _get_layer_id(name)
            if p.ndim == 1 or name.endswith('.bias') or name.endswith('_embed') or name.endswith('_token'):
                if name.startswith('patch_embed'):
                    patch_embed_not_regularized[layer_id].append((name, p))
                else:
                    not_regularized[layer_id].append((name, p))
            else:
                if name.startswith('patch_embed'):
                    patch_embed_regularized[layer_id].append((name, p))
                else:
                    regularized[layer_id].append((name, p))

    def _get_optim_params(d, group_name, apply_wd=True, is_patch_embed=False):
        if utils.is_main_process():
            for k, v in d.items():
                for vv in v:
                    logger.info(f'optimizer_group_created: {group_name}_{k} with {vv[0]}')
        return [dict(
            params=[vv[1] for vv in v], weight_decay=0.0, layer_decay=layer_decay ** (max_depth + 1 - k),
            apply_wd=apply_wd, is_patch_embed=is_patch_embed, group_name=f'{group_name}_{k}') for k, v in d.items()]

    optim_params = _get_optim_params(regularized, 'Regularized', True, False) + \
                   _get_optim_params(not_regularized, 'NotRegularized', False, False) + \
                   _get_optim_params(patch_embed_regularized, 'PatchEmbedRegularized', True, True) + \
                   _get_optim_params(patch_embed_not_regularized, 'PatchEmbedNotRegularized', False, True)
    optim_params = [x for x in optim_params if len(x["params"])]
    if zero == 'none':
        return torch.optim._multi_tensor.AdamW(optim_params, lr=0.0)
    return OSS(optim_params, torch.optim._multi_tensor.AdamW, lr=0.0)
            ...
            for name_p, p in m.named_parameters(recurse=False):
                ...
                if not p.requires_grad:
                    ...
                    zero_parameters.append(p)
                        ...
                        len(p.shape) == 1 or name.endswith(".bias") or len(p.squeeze().shape) == 1
                ):
                    zero_parameters.append(p)
                else:
