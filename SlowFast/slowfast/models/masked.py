...
class MaskMViT(MViT):
    ...
    def _mae_random_masking(self, x, mask_ratio, mask_in=None):
        ...
        if mask_in is None:
            ...
            noise = mask_in.flatten(1).float()
            mask_ratio = sum(noise.flatten()) / noise.numel()  # alrdy masked
        if mask_ratio == 0.0:
            # ids_keep = torch.arange(L).unsqueeze(0).expand(N, -1).to(x.device)
    ...
    def _mae_forward_encoder(self, x, mask_ratio, mask=None):
        ...
        if self.cls_embed_on:
            # append cls token
            cls_token = self.cls_token.type_as(x)
            if self.use_fixed_sincos_pos:
        ...
        if self.use_abs_pos and not self.use_fixed_sincos_pos:
            if self.sep_pos_embed:
                ...
                pos_embed = pos_embed.expand(x.shape[0], -1, -1).type_as(x)
