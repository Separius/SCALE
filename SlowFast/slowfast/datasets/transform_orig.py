...
def detailed_random_crop(images, size, boxes=None):
    if images.shape[2] == size and images.shape[3] == size:
        return images, 0, 0, size, size
    height = images.shape[2]
    width = images.shape[3]
    y_offset = 0
    if height > size:
    ...
    return cropped, y_offset, x_offset, y_offset+size, x_offset+size
...
def detailed_uniform_crop(images, size, spatial_idx, boxes=None, scale_size=None):
...
def detailed_horizontal_flip(prob, images):
    flip = np.random.uniform() < prob
    if flip:
        images = images.flip((-1))
    return images, 1.0 if flip else -1.0


def detailed_random_resized_crop(
        images,
        target_height,
        target_width,
        scale=(0.8, 1.0),
    ...
    return torch.nn.functional.interpolate(
        cropped, size=(target_height, target_width), mode="bilinear", align_corners=False), i, j, i + h, j + w
