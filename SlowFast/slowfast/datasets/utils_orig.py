...
def spatial_sampling(
    ...
    if spatial_idx == -1:
        if aspect_ratio is None and scale is None:
            ...
            _, _, h_scaled, w_scaled = frames.size()
            frames, h0, w0, h1, w1 = transform.detailed_random_crop(frames, crop_size)
            o = frames, h0*h/h_scaled, w0*w/w_scaled, h1*h/h_scaled, w1*w/w_scaled
        else:
            o = transform.detailed_random_resized_crop(
                images=frames,
        ...
        frames, h0, w0, h1, w1 = o
        frames, flipped = transform.detailed_horizontal_flip(0.5 if random_horizontal_flip else 0.0, frames)
        return frames, flipped, h0, w0, h1, w1
    else:
        # The testing is deterministic and no jitter should be performed.
        # min_scale, max_scale, and crop_size are expect to be the same.
        assert len({min_scale, max_scale}) == 1
        frames, _ = transform.random_short_side_scale_jitter(frames, min_scale, max_scale)  # deterministic
        o = transform.detailed_uniform_crop(frames, crop_size, spatial_idx)
        frames, h0, w0, h1, w1 = o
        return frames, -1, h0, w0, h1, w1
