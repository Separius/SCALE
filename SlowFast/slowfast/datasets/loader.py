...
def simplified_scale_train_collate(batch):
    result = {'label': default_collate([label for views, pos_info, label, index in batch]),
              'index': default_collate([index for views, pos_info, label, index in batch]),
              'pos_info': default_collate([pos_info for views, pos_info, label, index in batch])}  # B, SUM(Ni)xM, 7
    for frames in default_collate([views for views, pos_info, label, index in batch]):  # [Batched Tensors]
        _, _, time, height, _ = frames.size()
        key = f'{time}_{height}'
        if key not in result:
            result[key] = []
        result[key].append(frames)
    return result  # {'label': (B,), 'index': (B,), (16, 224): [(B, C, T, H, W)]xN, (16, 96): [(B, C, T, H, W)]xM, ...}


def scale_eval_collate(batch):
    result = {'view': default_collate([view for view, _, _ in batch]),
              'label': default_collate([label for _, label, _ in batch]),
              'index': default_collate([index for _, _, index in batch])}
    return result  # {'view': Tensor of size B, 3, T, H, W, 'label': Tensor of size B, 'index': Tensor of size B}
