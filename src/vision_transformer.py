...
def get_mugs(ckpt_path=None):
    model = vit_base()
    if ckpt_path is not None:
        model.load_state_dict(torch.load(ckpt_path))
    else:  # download https://drive.google.com/file/d/1M41TVVFyVRDTK5kbgLCEImrxw0AVtebb/view?usp=sharing
        state_dict = torch.load('./initialization/mugs_base400_imagenet.pth')['teacher']
        bl = len('backbone.')
        state_dict = {k[bl:]: v for k, v in state_dict.items() if not k.startswith('backbone.relation_blocks')
