...
from mugs.vision_transformer import trunc_normal_
...
def get_ibot(ckpt_path=None):
    model = vit_base()
    model.head = iBOTHead(
        model.embed_dim, 8192, patch_out_dim=8192, norm=None, act='gelu', shared_head=True)
    if ckpt_path is not None:
        model.load_state_dict(torch.load(ckpt_path))
    else:
        state = torch.hub.load_state_dict_from_url(
            'https://lf3-nlp-opensource.bytetos.com/obj/nlp-opensource/archive/2022/ibot/vitb_16_rand_mask/checkpoint.pth',
            model_dir='./initialization/', file_name='ibot_base400_imagenet.pth')['teacher']
        bl = len('backbone.')
        state_dict = {k[bl:] if k.startswith('backbone.') else k: v for k, v in state.items()}
