...
def init_distributed_mode(args, timeout=1800):
    ...
    # we manually add MASTER_ADDR and MASTER_PORT to env variables
        ...
        sys.exit(1)
    dist.init_process_group(backend="nccl", init_method=args.dist_url, world_size=args.world_size, rank=args.rank, timeout=datetime.timedelta(seconds=timeout))
    torch.cuda.set_device(args.gpu)
    dist.barrier()
    setup_for_distributed(args.rank == 0)


def move_to_gpu(thing):
    if isinstance(thing, tuple):
        return (move_to_gpu(x) for x in thing)
    elif isinstance(thing, list):
        return [move_to_gpu(x) for x in thing]
    elif isinstance(thing, dict):
        return {k: move_to_gpu(v) for k, v in thing.items()}
    else:
